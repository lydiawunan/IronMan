...
for k in range(len(case_index)):
    name='case_'+str(case_index[k])+'/DFG_case_'+str(case_index[k])+'.txt'
    f = open(name, "r")
    lines = f.readlines()
    print(case_index[k])
...
lut_target_pd.to_csv('graph_target_lut.csv',index=False)
dsp_target_pd.to_csv('graph_target_dsp.csv',index=False)
cp_target_pd.to_csv('graph_target_cp.csv',index=False)